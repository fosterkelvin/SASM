...
export const createRequirementsSubmission = catchErrors(
  async (req: Request, res: Response) => {
    ...
    // Process items and build complete mappedItems array preserving ALL items
    const mappedItems: any[] = [];

    for (let originalIndex = 0; originalIndex < items.length; originalIndex++) {
      const it = items[originalIndex];
      const jsonItem = fullItemsMap.get(originalIndex);

      // Check if this item has a new file upload
      const hasNewFile = it && (it.hasFile === "1" || it.hasFile === 1);
      ...
      if (hasNewFile) {
        ...
        if (file) {
          ...
          if (url) {
            const label = it.label || (jsonItem?.text || jsonItem?.label) || `Item ${originalIndex + 1}`;
            const clientId = it.clientId || jsonItem?.id;
        ...
        console.debug("[requirements][map] no file found for new upload", {
          originalIndex,
          expectedFieldname: `file_item_${originalIndex}`,
          label: it.label,
        });
      }

      // No new upload - check if we should preserve existing file from jsonItem
      if (jsonItem) {
        if (jsonItem.file && jsonItem.file.url && !jsonItem.file.url.startsWith("data:")) {
          // Preserve existing remote file
          ...
          // No file yet - preserve as placeholder without URL
          console.debug("[requirements][map] preserving item without file", {
            originalIndex,
            label: it?.label || jsonItem.text,
          });
        ...
        // No jsonItem but we have form data - create placeholder
        console.debug("[requirements][map] creating placeholder from form data", {
          originalIndex,
          label: it.label,
        });

        mappedItems.push({
          label: it.label || `Item ${originalIndex + 1}`,
          note: it.note,
          url: null,
          publicId: null,
          originalName: null,
    ...
    // Filter out items without URLs unless this is a resubmit (resubmits preserve all items)
    const finalMappedItems = isResubmit
      ? mappedItems  // Keep all items including placeholders for resubmits
      : mappedItems.filter((item) => item.url);  // Only keep items with files for initial submission

    console.debug("[requirements] finalMappedItems count:", {
      total: mappedItems.length,
      withFiles: finalMappedItems.length,
      isResubmit,
    });

    // If there are uploaded files but finalMappedItems is empty, create items from files directly
    if (finalMappedItems.length === 0 && files.length > 0) {
      const filesAsItems: any[] = files
        ...
        .filter((it) => it.url);
      // use these as finalMappedItems
      if (filesAsItems.length > 0) finalMappedItems.push(...filesAsItems);
    }
    ...
    if (alreadySubmitted && isResubmit) {
      ...
      try {
        ...
        // Since finalMappedItems now contains ALL items (both new and preserved), we can directly use it
        // but we need to ensure we delete any old files that are being replaced
        for (const newIt of finalMappedItems) {
          if (!newIt) continue;
        ...
        // Replace the entire items array with finalMappedItems (which contains the complete, updated set)
        alreadySubmitted.items = finalMappedItems;
    ...
    let finalItems = finalMappedItems;
    if (existingDraft) {
